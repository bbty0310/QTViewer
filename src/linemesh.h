...
inline BV::BV(vector<Face> fcs, int lv, SurfaceMesh& mesh) {
    ...
    // 분할 기준 축
    double splitValueX = 0.5 * (minPoint[0] + maxPoint[0]);
    double splitValueY = 0.5 * (minPoint[1] + maxPoint[1]);
    double splitValueZ = 0.5 * (minPoint[2] + maxPoint[2]);
    ...
    if (!leftFacesX.empty() && !rightFacesX.empty())
    {
        // x 축에 대한 분할
        left_ = new BV(leftFacesX, level + 1, mesh);
        right_ = new BV(rightFacesX, level + 1, mesh);
    }
    else if (!leftFacesY.empty() && !rightFacesY.empty())
    {
        // y 축에 대한 분할
